...
test('Deactivate seller account', async ({ request }) => {
    ...
    const response = await accountsRequest.deactivateSellerAccount(marketplaceData.markeplaceId, sellerName, sellerEmail);
    console.log(await response.json());
    expect(response.status()).toBe(201);
    const responseData = await response.json();
    expect(responseData.data[0].status).toBe('deactivated');
});